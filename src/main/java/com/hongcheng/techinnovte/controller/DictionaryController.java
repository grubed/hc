...
import com.hongcheng.techinnovte.pojo.dto.AttachMoreDTO;
...
//    @GetMapping("/dictionary/o")
//    public JsonResult getViewObjectList(Dictionary entity) throws Exception {
//        QueryWrapper<Dictionary> queryWrapper = super.buildQueryWrapper(entity);
//        List<Dictionary> voList = dictionaryService.getEntityList(queryWrapper);
//        Map<String, List<Dictionary>> map = voList.stream().collect(Collectors.groupingBy(Dictionary::getType));
//        List<Dictionary> ret = new ArrayList<>();
//        for (Map.Entry<String, List<Dictionary>> entry : map.entrySet()) {
//            List<Dictionary> v = entry.getValue();
//            Long p = 0L;
//            for(Dictionary d : v) {
//                if(d.getItemValue() != null && d.getItemValue().equals("!")) {
//                    p = d.getId();
//                    break;
//                }
//            }
...
//
//        }
//        dictionaryService.createOrUpdateEntities(ret);
...
//    }
    ...
    @ApiOperation(value = "获取dictionary")
    @GetMapping("/dictionary")
    public JsonResult getTypeDuplicate(@RequestParam String type) {
        if (V.notEmpty(type)) {
            LambdaQueryWrapper<Dictionary> wrapper = new LambdaQueryWrapper();
            wrapper.select().eq(Dictionary::getType, type).gt(Dictionary::getParentId, 0);

            return JsonResult.OK(dictionaryService.getEntityList(wrapper));
        }
        return JsonResult.OK();
    }
    /**
     ...
     * @return
     */
    @ApiOperation(value = "获取附加属性的通用kvList接口，用于初始化前端下拉框选项")
    ...
    public JsonResult attachMore(@Valid @RequestBody ValidList<AttachMoreDTO> attachMoreDTOList) {
        ...
        for (AttachMoreDTO attachMoreDTO : attachMoreDTOList) {
            ...
            if (type.equals(AttachMoreDTO.REF_TYPE.D)) {
                if(attachMoreDTO.getValue() != null) {
                    QueryWrapper<Dictionary> wrapper2 = new QueryWrapper<>();
                    wrapper2.eq("type", attachMoreDTO.getTarget());
                    wrapper2.eq("parent_id", attachMoreDTO.getValue());
                    List<KeyValue> keyValueList = dictionaryService.getKeyValueList(wrapper2.select("item_name","item_value")); //.getKeyValueList(attachMoreDTO.getTarget());
